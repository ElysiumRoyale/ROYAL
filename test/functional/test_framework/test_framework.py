...
TMPDIR_PREFIX = "elysium_royale_func_test_"


class Elysium_RoyaleTestFramework():
    """Base class for a elysium_royale test script.

    Individual elysium_royale test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave elysium_royaleds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop elysium_royaleds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing elysium_royaled/elysium_royale-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use elysium_royale-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: elysium_royaleds were not stopped and may still be running")
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple elysium_royaleds"""
    ...
    def stop_node(self, i):
        """Stop a elysium_royaled test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple elysium_royaled test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as elysium_royaled's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in elysium_royale.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in elysium_royale.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "elysium_royaled"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run elysium_royaleds:
    ...
    # Elysium_Royale Specific TestFramework
    ...
    """
    Create a ProReg tx, which references an 100 ROYAL UTXO as collateral.
...
'''
Elysium_RoyaleTestFramework extensions
'''
...
class Elysium_RoyaleDMNTestFramework(Elysium_RoyaleTestFramework):
...
# !TODO: remove after obsoleting legacy system
class Elysium_RoyaleTier2TestFramework(Elysium_RoyaleTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [["-nuparams=v5_shield:249", "-nuparams=Elysium_Royale_v5.5:250", "-nuparams=v6_evo:250", "-whitelist=127.0.0.1"]] * self.num_nodes
