...
class ReorgStakeTest(Elysium_RoyaleTestFramework):
    ...
    def check_money_supply(self, expected_royal):
        # verify that nodes have the expected ROYAL supply
        royal_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(royal_supply, [DecimalAmt(expected_royal)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that ROYAL supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check ROYAL supply...")
