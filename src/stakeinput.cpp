...
CRoyalStake* CRoyalStake::NewRoyalStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CRoyalStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CRoyalStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CRoyalStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CRoyalStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CRoyalStake::GetValue() const
{
    return outputFrom.nValue;
}

CDataStream CRoyalStake::GetUniqueness() const
{
    //The unique identifier for a ROYAL stake is the outpoint
...
const CBlockIndex* CRoyalStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CRoyalStake: uninitialized pindexFrom");
    return pindexFrom;
}
