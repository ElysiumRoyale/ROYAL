...
struct Params {
    ...
    // validation by-pass
    int64_t nElysium_RoyaleBadBlockTime;
    unsigned int nElysium_RoyaleBadBlockBits;
    ...
    CAmount GetMNCollateral(int nHeight) const
    {
        ...
        if (nHeight > 0 && nHeight <= 250) {
            nMNCollateral = 0;
        } else if (nHeight > 250 && nHeight <= 1000) {
            nMNCollateral = 1000;
            ...
            nMNCollateral = 2500000;
        } else {
            nMNCollateral = 5000000;
        }
