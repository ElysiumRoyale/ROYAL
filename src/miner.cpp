...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("Elysium_RoyaleMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("Elysium_RoyaleMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Elysium_RoyaleMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("elysium_royale-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Elysium_RoyaleMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("Elysium_RoyaleMiner exception");
    } catch (...) {
        LogPrintf("Elysium_RoyaleMiner exception");
    }

    LogPrintf("Elysium_RoyaleMiner exiting\n");
}
