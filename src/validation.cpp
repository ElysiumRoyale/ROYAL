...
#if defined(NDEBUG)
#error "Elysium_Royale cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uroyal) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k ROYAL
    ...
    if (nHeight > 0 && nHeight < nLast) {
        return 800 * COIN;
    } else if (nHeight >= nLast && nHeight <= 100000) {
        return 4 * COIN;
...
{
    util::ThreadRename("elysium_royale-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Elysium_Royale Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nElysium_RoyaleBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nElysium_RoyaleBadBlockBits)) {
            // accept Elysium_Royale block minted with incorrect proof of work threshold
            return true;
        }
