...
const char * const Elysium_Royale_CONF_FILENAME = "elysiumroyale.conf";
const char * const Elysium_Royale_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Elysium_Royale only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "elysium_royaled -foo=bar
...
#else
    const char* pszModule = "elysium_royale";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Elysium_Royale
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Elysium_Royale
// Mac: ~/Library/Application Support/Elysium_Royale
// Unix: ~/.elysium_royale
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElysiumRoyale";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Elysium_RoyaleParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Elysium_RoyaleParams
    // Mac: ~/Library/Application Support/Elysium_RoyaleParams
    // Unix: ~/.elysium_royale-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElysiumRoyaleParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "ElysiumRoyaleParams";
#else
    // Unix
    return pathRet / ".elysiumroyale-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/elysium_royale/sapling-spend.params";
        sapling_output = "/usr/share/elysium_royale/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/elysium_royale/sapling-spend.params";
            sapling_output = "/usr/local/share/elysium_royale/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", Elysium_Royale_MASTERNODE_CONF_FILENAME));
