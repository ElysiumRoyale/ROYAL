 ...
 */
const char * const Elysium_Royale_PID_FILENAME = "elysium_royale.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", Elysium_Royale_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("elysium_royale-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", Elysium_Royale_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", Elysium_Royale_PID_FILENAME));
#endif
...
{
    util::ThreadRename("elysium_royale-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that Elysium_Royale is running in a usable environment with all
...
#endif
    LogPrintf("Elysium_Royale version %s\n", version_string);
}
...
{
    // Make sure only a single Elysium_Royale process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", Elysium_Royale_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if Elysium_Royale is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if Elysium_Royale is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //Elysium_Royale specific: spork DB's
                ...
                // Elysium_Royale: load previous sessions sporks if we have them.
