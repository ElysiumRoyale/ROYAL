...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select ROYAL Outputs to Spend" :
            "Select Shielded ROYAL to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::ROYAL;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show Elysium_Royale address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
