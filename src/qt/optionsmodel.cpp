...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ROYAL);
    ...
    if (settings.contains("nAnonymizeElysium_RoyaleAmount") || reset)
        gArgs.SoftSetArg("-anonymizeelysium_royaleamount", settings.value("nAnonymizeElysium_RoyaleAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in elysium_royale.cpp during shotdown to also remove the window positions
