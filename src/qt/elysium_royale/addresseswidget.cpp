...
#include "qt/elysium_royale/forms/ui_addresseswidget.h"
#include "qt/elysium_royale/addressfilterproxymodel.h"
#include "qt/elysium_royale/addresslabelrow.h"
#include "qt/elysium_royale/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(Elysium_RoyaleGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto royalAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(royalAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(royalAdd);
        if (walletModel->updateAddressBookLabels(royalAdd, label.toUtf8().constData(),
