...
#include "qt/elysium_royale/forms/ui_snackbar.h"
#include "qt/elysium_royale/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(Elysium_RoyaleGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &Elysium_RoyaleGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
