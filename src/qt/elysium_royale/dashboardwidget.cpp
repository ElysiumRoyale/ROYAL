...
#include "qt/elysium_royale/dashboardwidget.h"
#include "qt/elysium_royale/forms/ui_dashboardwidget.h"
#include "qt/elysium_royale/sendconfirmdialog.h"
#include "qt/elysium_royale/txrow.h"
...
DashboardWidget::DashboardWidget(Elysium_RoyaleGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareRoyal, "square-chart-royal");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelRoyal, "text-chart-royal");
    ...
    setCssProperty(ui->labelAmountRoyal, "text-stake-royal-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &Elysium_RoyaleGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ROYAL")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(23,18,11);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isRoyal = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ROYAL, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            royal = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalRoyal += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesRoyal.append(royal);
        chartData->valuesMN.append(mn);

        int max = std::max(royal, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(180,128,54));
    set1->setColor(QColor(252,203,134));
    ...
    set0->append(chartData->valuesRoyal);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalRoyal > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountRoyal, "text-stake-royal");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountRoyal, "text-stake-royal-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountRoyal, ui->labelAmountMN});
    ui->labelAmountRoyal->setText(GUIUtil::formatBalance(chartData->totalRoyal, nDisplayUnit));
