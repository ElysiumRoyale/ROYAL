...
#include "qt/elysium_royale/forms/ui_masternodeswidget.h"

#include "qt/elysium_royale/qtutils.h"
#include "qt/elysium_royale/mnrow.h"
#include "qt/elysium_royale/mninfodialog.h"
#include "qt/elysium_royale/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(Elysium_RoyaleGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the elysiumroyale.conf file\nof your remote server and start it, "
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a masternode, %1 required.")
            .arg(GUIUtil::formatBalance(mnCollateralAmount, BitcoinUnits::ROYAL)));
        return;
    }
    showHideOp(true);
