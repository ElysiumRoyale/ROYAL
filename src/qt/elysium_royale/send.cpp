...
#include "qt/elysium_royale/send.h"
#include "qt/elysium_royale/forms/ui_send.h"
#include "qt/elysium_royale/addnewcontactdialog.h"
#include "qt/elysium_royale/qtutils.h"
#include "qt/elysium_royale/sendchangeaddressdialog.h"
#include "qt/elysium_royale/optionbutton.h"
#include "qt/elysium_royale/sendconfirmdialog.h"
#include "qt/elysium_royale/guitransactionsutils.h"
#include "qt/elysium_royale/loadingdialog.h"
...
SendWidget::SendWidget(Elysium_RoyaleGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a Elysium Royale URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-royal");
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent ROYALs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield ROYALs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent ROYALs to shield."));
    }
}
...
void SendWidget::onROYALSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto royalAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(royalAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(royalAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(royalAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
