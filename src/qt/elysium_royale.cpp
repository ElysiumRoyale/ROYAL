...
#include "qt/elysium_royale/splash.h"
#include "qt/elysium_royale/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/elysium_royale/governancemodel.h"
...
{
    return QCoreApplication::translate("elysium_royale-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in elysium_royale.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in elysium_royale.qrc)
...
/** Class encapsulating Elysium_Royale Core startup and shutdown.
...
/** Main Elysium_Royale application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (Elysium_RoyaleGUI)
...
private:
    ...
    Elysium_RoyaleGUI* window{nullptr};
...
{
    window = new Elysium_RoyaleGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &Elysium_RoyaleGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &Elysium_RoyaleGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(Elysium_RoyaleGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(Elysium_RoyaleGUI::DEFAULT_WALLET);
        ...
        // Elysium_Royale: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &Elysium_RoyaleGUI::handlePaymentRequest);
        connect(window, &Elysium_RoyaleGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. Elysium_Royale can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(elysium_royale_locale);
    Q_INIT_RESOURCE(elysium_royale);
    ...
    /// 6. Determine availability of data directory and parse elysiumroyale.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", Elysium_Royale_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // elysium_royale: links repeatedly have their payment requests routed to this process:
