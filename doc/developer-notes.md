...
- [Development guidelines](#development-guidelines)
    - [General Elysium_Royale Core](#general-elysium_royale-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Elysium_Royale Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ROYALs (tROYAL)" on the test network, if you
...
Elysium_Royale Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/elysium_royale-project/elysium_royale/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_elysium_royale
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_elysium_royale --log_level=test_suite
$ valgrind -v --leak-check=full src/elysium_royaled -printtoconsole
```
...
# A coverage report will now be accessible at `./test_elysium_royale.coverage/index.html`.
```

**Sanitizers**
Elysium_Royale can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates ROYALs (if wallet is enabled).
...
However, in open source software such as Elysium_Royale Core, where everyone uses
...
pay attention to for reviewers of Elysium_Royale Core code.

General Elysium_Royale Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Elysium_Royale Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Elysium_Royale Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/Elysium_Royale-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with Elysium_Royale's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Elysium_Royale-Project/Elysium_Royale.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `elysium_royale-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `elysium_royale-cli` and the GUI debug console use this table to determine how to
