Elysium_Royale Core version *v5.0.0* is now available from:  <https://github.com/elysium_royale-project/elysium_royale/releases>
...
Elysium_Royale Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, Elysium_Royale Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run Elysium_Royale v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving ROYALs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://elysium_royale.org .
...
* Top bar showing shield ROYAL balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield ROYAL balance.
...
See the release notes from [v4.2.0](https://github.com/Elysium_Royale-Project/Elysium_Royale/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed ROYAL supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in ROYAL
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in ROYAL
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zROYALsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield ROYAL)
  The `moneysupply` JSON return entry is the total ROYAL supply (the sum of all transparent and shield ROYAL)
...
| `getaccountaddress` | Returned the current Elysium_Royale address associated with an internal account | Account system removal |
| `setaccount` | Associate a Elysium_Royale address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given Elysium_Royale address | Account system removal |
| `getaddressesbyaccount` | Returned a list of Elysium_Royale addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a Elysium_Royale address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a Elysium_Royale address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zROYAL balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zROYAL mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zROYAL mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zROYAL amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zROYAL to a ROYAL address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zROYAL mints to a ROYAL address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zROYALs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzroyalseed` | Set the wallet's deterministic zROYAL seed to a specific value | Zerocoin deprecation |
| `getzroyalseed` | Return the current deterministic zROYAL seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zROYAL seed | Zerocoin deprecation |
| `dzroyalstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzroyal` | Do a search for deterministically generated zROYAL that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_elysium_royale unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake elysium_royaled and unit tests: fixing missing MacOS framework. (furszy)
