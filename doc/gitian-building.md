...
*Setup instructions for a gitian build of Elysium_Royale Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Elysium_Royale
...
to the Elysium_Royale GitHub release page.
...
- [Building Elysium_Royale Core](#building-elysium_royale-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/elysium_royale-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/elysium_royale-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/elysium_royale-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/elysium_royale-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/elysium_royale-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/elysium_royale-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/elysium_royale-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@elysium_royale.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/elysium_royale-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/elysium_royale-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/elysium_royale-osx-*-build.assert
```
